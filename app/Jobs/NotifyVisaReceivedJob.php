...
class NotifyVisaReceivedJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach($this->visaProcessId ?? [] as $visaId){
            ...
            if($receiver){
                ...
                $title = $this->type." Received ";
                $web_content = 'Your  '.$this->type.' has been received from '.$company->name.' for demand '. $demand->demand_code.' <a href="'.@$go_to_url.'">View More</a>';
                $mobile_content = 'Your  '.$this->type.' has been received from '.$company->name.' for demand '. $demand->demand_code.' <a href="'.@$go_to_url.'">View More</a>';
                $generated_by = "System";
                $generated_id = null;
