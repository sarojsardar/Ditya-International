                                    ...
                                    @forelse($demands as $demand)
                                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="list-demand{{ $demand->id }}" role="tabpanel">
                                            ...
                                            <div class="table-responsive mt-3">
                                                <table class="table table-bordered dt-responsive nowrap" id="matchedCandidatesTable{{ $demand->id }}" data-demand-code="{{ $demand->demand_code }}">
                                                    <thead>
                                                    <tr>
                                                        <th>S.N</th>
                                                        <th>Status</th>
    ...
    <script>
        ...
        function initDataTableForActiveTab() {
            ...
            if (!$.fn.DataTable.isDataTable('#' + tableId)) {
                ...
                var matchedCandidatesUrl = "{{ route('candidate.demandCandidates', ':demandCode') }}".replace(':demandCode', demandCode) + '?type=matched';

                // Initialize DataTable with the dynamic URL and configurations
                initDataTable(tableId, matchedCandidatesUrl);
            } else {
