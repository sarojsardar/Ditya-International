...
@section('content')
                ...
                <div class="card-body">
                    <form action="#">
                       <div class="row">
                            ...
                            <div class="col-md-12">
                                <button type="button" class="btn btn-primary btn-sm" id="btn-cnage-visa-process">Change E Visa Status</button>
                            </div>
                       </div>
                    </form>
                </div>
    ...
    <div class="modal fade" id="procees-to-visa-process" tabindex="-1" role="dialog" aria-labelledby="procees-to-visa-processLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form action="{{route('company-officer.proceed-to-evisa')}}" method="post" id="procees-to-visa-process" enctype="multipart/form-data">
                @csrf
...
@push('scripts')
    ...
    <script>
        ...
        let table = $('#company-list-datatable').DataTable({
            ...
            "severside": true,
            ajax: {
                url: "{{route('company-officer.candidate-data')}}",
                data: function(d) {
                    d.type = "{{ @$type }}";
                    d.demand = $('#demand').val();
                ...
                { 
                    ...
                    "render": function(data, type, row) {
                        ...
                        return (
                            ...
                            (row.visa_status !== null && row.visa_status !== '' && row.visa_status !== undefined && row.visa_status == "Successed")  && 
                            (row.evisa_status !== null && row.evisa_status !== '' && row.evisa_status !== undefined && row.evisa_status !== "Successed" && row.evisa_status !== "Rejected") &&
                            (row.demand_status !== "Close"  && row.demand_status !== "Completed")) ? 
                            `<input type="checkbox" name="selectedCandidates[]" value="${data}">` : '';
                    }
                },
                {
                    'data':'evisa_status'
